...
@section('content')
    ...
    <form action="{{ route('admin.helionix.announcement.edit', $announcement->id) }}" method="POST">
        @csrf
        <div class="form-group">
            <label for="title" class="control-label">Title</label>
            <div>
                <input type="text" id="title" name="title" class="form-control" value="{{ old('title', $announcement->title) }}" />
        ...
        <div class="form-group">
            ...
            <div>
                <textarea rows="4" id="description" name="description" class="form-control">{{ old('description', $announcement->description) }}</textarea>
                <p class="text-muted"><small>The description for announcement.</small></p>
            </div>
        </div>
        <div class="form-group">
            <button class="btn btn-content">Update</button>
        </div>
    </form>
