...
@section('content-header')
    <h1>Edit Schedule Template<small>Update the details of the schedule template</small></h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('admin.index') }}">Admin</a></li>
        <li><a href="{{ route('admin.veltastudios.schedule-templates') }}">Schedule Template Manager</a></li>
        <li class="active">Edit Schedule Template</li>
    </ol>
@endsection
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="box box-primary">
                ...
                <form action="{{ route('admin.veltastudios.schedule-templates.update', $template->id) }}" method="POST">
                    @csrf
                    @method('PATCH')
                    <div class="box-body">
                        <h4>Schedule</h4>
                        ...
                        <div class="form-group">
                            ...
                            <div class="row">
                                <div class="form-group col-md-2 col-xs-6">
                                    ...
                                    <input type="text" name="cron[minute]" class="form-control" value="{{ $template->cron['minute'] }}" placeholder="*" required>
                                </div>
                                <div class="form-group col-md-2 col-xs-6">
                                    <label for="cron-hour">Hour</label>
                                    <input type="text" name="cron[hour]" class="form-control" value="{{ $template->cron['hour'] }}" placeholder="*" required>
                                </div>
                                <div class="form-group col-md-2 col-xs-6">
                                    <label for="cron-dayOfMonth">Day of Month</label>
                                    <input type="text" name="cron[dayOfMonth]" class="form-control" value="{{ $template->cron['dayOfMonth'] }}" placeholder="*" required>
                                </div>
                                <div class="form-group col-md-2 col-xs-6">
                                    <label for="cron-month">Month</label>
                                    <input type="text" name="cron[month]" class="form-control" value="{{ $template->cron['month'] }}" placeholder="*" required>
                                </div>
                                <div class="form-group col-md-2 col-xs-6">
                                    <label for="cron-dayOfWeek">Day of Week</label>
                                    <input type="text" name="cron[dayOfWeek]" class="form-control" value="{{ $template->cron['dayOfWeek'] }}" placeholder="*" required>
                                </div>
                            </div>
                        </div>
                            ...
                            <table class="table table-hover">
                                ...
                                <tbody id="tasks-container">
                                    @foreach ($template->tasks as $index => $task)
                                        <tr class="task">
                                            <td>
                                                <select name="tasks[{{ $index }}][action]" class="form-control action-select" required>
                                                    <option value="command" {{ $task->action === 'command' ? 'selected' : '' }}>Send Command</option>
                                                    <option value="power" {{ $task->action === 'power' ? 'selected' : '' }}>Send Power Action</option>
                                                    <option value="backup" {{ $task->action === 'backup' ? 'selected' : '' }}>Create Backup</option>
                                                </select>
                                            </td>
                                            <td>
                                                @if ($task->action === 'power')
                                                    <select name="tasks[{{ $index }}][payload]" class="form-control payload-input">
                                                        <option value="start" {{ $task->payload === 'start' ? 'selected' : '' }}>Start the server</option>
                                                        <option value="restart" {{ $task->payload === 'restart' ? 'selected' : '' }}>Restart the server</option>
                                                        <option value="stop" {{ $task->payload === 'stop' ? 'selected' : '' }}>Stop the server</option>
                                                        <option value="terminate" {{ $task->payload === 'terminate' ? 'selected' : '' }}>Terminate the server</option>
                                                    </select>
                                                @elseif ($task->action === 'backup')
                                                    <input type="text" name="tasks[{{ $index }}][payload]" class="form-control payload-input" value="{{ $task->payload }}" placeholder="Ignored files (optional)">
                                                @else
                                                    <input type="text" name="tasks[{{ $index }}][payload]" class="form-control payload-input" value="{{ $task->payload }}" required>
                                                @endif
                                            </td>
                                            <td>
                                                <input type="number" name="tasks[{{ $index }}][timeOffset]" class="form-control" value="{{ $task->time_offset }}" required>
                                            </td>
                                            <td>
                                                <select name="tasks[{{ $index }}][continueOnFailure]" class="form-control" required>
                                                    <option value="1" {{ $task->continue_on_failure ? 'selected' : '' }}>Yes</option>
                                                    <option value="0" {{ !$task->continue_on_failure ? 'selected' : '' }}>No</option>
                                                </select>
                                            </td>
                                            <td>
...
@section('footer-scripts')
    @parent
    <script>
        let taskIndex = {{ count($template->tasks) }};
