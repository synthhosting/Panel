    ...
    return (
        <PageContentBlock title={'Dashboard'} showFlashKey={'dashboard'}>
            <div css={tw`w-full`}>
                <div css={tw`flex flex-wrap gap-2 mb-2`}>
                    {dash.filter(dash => dash.status == true).map((dash, index) => (
                        <a 
                            ...
                            css={tw`flex-1 h-20 bg-helionix-color2 rounded-xl p-4 hover:border-helionix-color3 border-2 border-transparent transition-colors duration-150`}
                        >
                ...
                {rootAdmin && (
                    <div css={tw`w-full content-center h-16 mb-2`}>
                        <div css={tw`flex justify-between items-center`}>
                            <p css={tw`uppercase text-sm`}>
