...
const ButtonStyle = styled.button<Omit<Props, 'isLoading'>>`
    ${tw`relative inline-block p-[12px] rounded-[15px] tracking-wide text-base font-semibold transition-all duration-150`};

    ${(props) =>
        ((!props.isSecondary && !props.color) || props.color === 'primary') &&
        css<Props>`
            ${(props) => !props.isSecondary && tw`bg-helionix-btnPrimary`};

            &:hover:not(:disabled) {
                ${tw`bg-helionix-btnPrimaryHover`};
            }
        `};
