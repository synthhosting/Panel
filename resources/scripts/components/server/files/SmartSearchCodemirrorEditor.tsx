...
export default ({ style, initialContent, filename, mode, onModeChanged, lines, query }: Props) => {
    ...
    useEffect(() => {
        ...
        if (editor) {
            lines.map((line, key) => {
                const lineContent = editor.getLine(line - 1);

                if (lineContent) {
                    // Make the text markers
                    editor.markText(
                        { line: line - 1, ch: lineContent.toString().toLowerCase().indexOf(query) },
                        {
                            line: line - 1,
                            ch: lineContent.toString().toLowerCase().indexOf(query) + query.length,
                        },
                        { css: 'background-color: #FFFF77;' }
                    );
                    ...
                    // Make sure not to hide before line part
                    let beforeEndLine = 0;
                    if (lines[key - 1]) {
                        beforeEndLine = lines[key - 1] + 3;
                    }

                    // Hide before
                    if (startLine > beforeEndLine) {
                        editor.markText(
                            { line: beforeEndLine, ch: 0 },
                            { line: startLine - 2, ch: 1000 },
                            { inclusiveRight: true, inclusiveLeft: true, collapsed: true }
                        );
                    }

                    // Make sure not to hide next line part
                    let nextStartLine = editor.lineCount();
                    if (lines[key + 1]) {
                        nextStartLine = lines[key + 1] - 3 - 2;
                    }
