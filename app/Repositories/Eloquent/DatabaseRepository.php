...
class DatabaseRepository extends EloquentRepository implements DatabaseRepositoryInterface
{
    ...
    /**
     * Get size of a given database on a specific connection.
     */
    public function getDatabaseSize(string $database): int
    {
        $connection = $this->database->connection($this->getConnection());
        $result = $connection->select(sprintf('SELECT SUM(data_length + index_length) AS size FROM information_schema.tables WHERE table_schema="%s" GROUP BY table_schema;', $database));
